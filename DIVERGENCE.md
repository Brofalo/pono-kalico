...
|-------|--------|------------------|-------------------|
| `0001-remove-save-config-subfile-check.patch` | pono-print-os meta-opencentauri/recipes-apps/klipper/files/ | foundation plan: conditional on cosmos config audit | audit cosmos config for actual duplicate include sections; if zero duplicates, drop patch; if non-zero, keep patch + document each duplicate the patch enables |
...
- The R2 cap (10 active Brofalo-only commits vs KalicoCrew main) is a
  soft trigger. Current count = 3 (the 3 unconditional patches migrated
  2026-05-23) plus 3 inherited from OC parent (afe7178d, f66de876,
  5f9fabbd) for a total of 6. Headroom = 4 before R2 fires.
